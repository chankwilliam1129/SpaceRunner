...
float4 main(VS_OUT pin) : SV_TARGET
{
	...
	float4 fragment_colour = hdr_colour_map.Sample(linear_border_sampler_state, pin.texcoord) * weight[0];
	for (int i = 1; i < 3; i++)
	{
		fragment_colour += hdr_colour_map.Sample(linear_border_sampler_state, pin.texcoord + float2(offset[i] / width, 0.0)) * weight[i];
		fragment_colour += hdr_colour_map.Sample(linear_border_sampler_state, pin.texcoord - float2(offset[i] / width, 0.0)) * weight[i];
	}
	...
	for (int i = 0; i < 2; i++)
	{
		fragment_colour += hdr_colour_map.Sample(linear_border_sampler_state, pin.texcoord + float2(offset[i], 0.0) / width) * weight[i];
		fragment_colour += hdr_colour_map.Sample(linear_border_sampler_state, pin.texcoord - float2(offset[i], 0.0) / width) * weight[i];
	}
#endif
