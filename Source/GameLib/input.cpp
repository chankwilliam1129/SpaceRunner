...
UINT Mouse::GetState(UINT msg)
{
	...
	{
	case WM_LBUTTONDOWN:
		mouse.SetBottonDownFlg(Botton::LEFT);
		break;

	case WM_LBUTTONUP:
		mouse.SetBottonUpFlg(Botton::LEFT);
		break;

	case WM_RBUTTONDOWN:
		mouse.SetBottonDownFlg(Botton::RIGHT);
		break;

	case WM_RBUTTONUP:
		mouse.SetBottonUpFlg(Botton::RIGHT);
		break;

	case WM_MBUTTONDOWN:
		mouse.SetBottonDownFlg(Botton::MIDDLE);
		break;

	case WM_MBUTTONUP:
		mouse.SetBottonUpFlg(Botton::MIDDLE);
		break;
	}
	return NULL;
}
...
Vector2 Mouse::GetPosition()
{
	POINT p;
	GetCursorPos(&p);
	ScreenToClient(GetSystemManager->hwnd, &p);
...
{
	if (isPush[botton]) { return true; }
	return false;
}

bool Mouse::PushTrigger(const Botton botton)
{
	if (isPushTrigger[botton])
	{
		isPushTrigger[botton] = false;
		return true;
	}
	return false;
}
...
{
	if (isReleaseTrigger[botton]) 
	{
		isReleaseTrigger[botton] = false;
		return true;
	}
	return false;
}
...
//
//HRESULT GamePad::CheckControllerState()
//{
//	DWORD dw_result = 0;
...
//}
//
//Vector2 GamePad::GetLStickVector(const PlayerID player)
//{
//	float normalizeLength = 32768.0f;	//-1～1に収めるための値
//
//	float x = static_cast<float>(gamePad[player].state.Gamepad.sThumbLX) / normalizeLength;
//	float y = static_cast<float>(gamePad[player].state.Gamepad.sThumbLY) / normalizeLength;
//
//	return Vector2(x, y);
//}
//
//Vector2 GamePad::GetRStickVector(const PlayerID player)
//{
//	float normalizeLength = 32768.0f;	//-1～1に収めるための値
//
//	float x = static_cast<float>(gamePad[player].state.Gamepad.sThumbRX) / normalizeLength;
//	float y = static_cast<float>(gamePad[player].state.Gamepad.sThumbRY) / normalizeLength;
//
//	return Vector2(x, y);
//}
//
