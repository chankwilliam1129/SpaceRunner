...
Font::Font(ID3D11Device* device, const char* filename, int maxSpriteCount)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Data/Shaders/CSO/font_vs.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Data/Shaders/CSO/font_ps.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
void Font::Draw(const wchar_t* string, Vector2 pos, Vector4 color)
{
	Draw(string, pos.x, pos.y, color.x, color.y, color.z, color.w);
}

void Font::Draw(const wchar_t* string, float x, float y, float r, float g, float b, float a)
{
	...
	for (size_t i = 0; i < length; ++i)
	{
		...
		currentVertex[0].color.x = r;
		currentVertex[0].color.y = g;
		currentVertex[0].color.z = b;
		currentVertex[0].color.w = a;
		...
		currentVertex[1].color.x = r;
		currentVertex[1].color.y = g;
		currentVertex[1].color.z = b;
		currentVertex[1].color.w = a;
		...
		currentVertex[2].color.x = r;
		currentVertex[2].color.y = g;
		currentVertex[2].color.z = b;
		currentVertex[2].color.w = a;
		...
		currentVertex[3].color.x = r;
		currentVertex[3].color.y = g;
		currentVertex[3].color.z = b;
		currentVertex[3].color.w = a;
